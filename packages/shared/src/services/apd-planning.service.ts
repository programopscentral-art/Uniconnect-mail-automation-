...
export async function parseAPDExcel(buffer: Buffer): Promise<APDParseResult> {
    ...
    // xlsx exports vary by environment — handle CJS default, ESM named, or nested default
    const xlsxRead = xlsxModule.read || xlsxModule.default?.read;
    const xlsxUtils = xlsxModule.utils || xlsxModule.default?.utils;
    if (!xlsxRead || !xlsxUtils) {
        throw new Error(`xlsx module loaded but read/utils not found. Keys: ${Object.keys(xlsxModule).join(', ')}`);
    }
