...
export default function ScanPage() {
  ...
  const startScanner = async () => {
    ...
    try {
      // Step 1: Force trigger the native browser pop-up
      try {
        await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } });
      } catch (e: any) {
        throw new Error(`Permission Denied: ${e.message}. Please click the "AA" or Camera icon in your browser to Allow.`);
      }

      // Step 2: Initialize scanner engine
      const H5 = (window as any).Html5Qrcode;
      if (!H5) throw new Error('System still initializing. Tap again in 1 second.');
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col text-white font-sans overflow-hidden">
      ...
      <div className="bg-gray-900 border-b border-white/5 px-4 py-4 flex items-center justify-between z-40 shadow-lg">
        <div className="flex flex-col">
          <p className="font-black text-2xl tracking-tighter leading-none">{currentTime}</p>
          <div className="flex items-center gap-2 mt-1">
            <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" />
            <p className="text-[10px] font-black uppercase tracking-widest text-green-500">System Live</p>
          </div>
        </div>
        <button onClick={() => setShowSettings(true)} className="p-3 bg-white/5 rounded-2xl border border-white/10 active:scale-90">
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center p-6 bg-gradient-to-b from-gray-900 to-black">
        ...
        <div className="w-full max-w-sm aspect-square relative mb-12">
          {!cameraActive ? (
            <button
              ...
              className="absolute inset-0 bg-indigo-600 rounded-[3rem] border-4 border-white/20 flex flex-col items-center justify-center p-10 cursor-pointer text-center shadow-[0_0_50px_rgba(79,70,229,0.4)] active:scale-95 transition-all"
            >
              ...
              <p className="text-2xl font-black text-white uppercase tracking-tighter">{initializing ? 'Requesting...' : 'Enable Camera'}</p>
              <p className="text-white/60 text-xs mt-2 font-bold uppercase tracking-widest leading-relaxed">Click to trigger the "Allow" pop-up</p>
            </button>
          ) : (
          ...
          {cameraError && (
            <div className="absolute -bottom-28 left-0 right-0 text-center px-4">
              <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl">
                <p className="text-red-500 font-black text-xs uppercase mb-2">Permission Required</p>
                <p className="text-gray-400 text-[10px] leading-tight font-medium">{cameraError}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="w-full max-w-sm mt-8">
          ...
          >
            ...
            />
            <button className="absolute right-2 top-2 bottom-2 bg-white text-black px-6 rounded-xl font-black text-xs uppercase tracking-widest">Send</button>
          </form>
          {deviceKey && (
            <p className="text-center text-[10px] text-gray-600 mt-6 font-bold uppercase tracking-widest">Device Key: {deviceKey.slice(0, 8)}...</p>
          )}
        </div>
      </div>

      {showSettings && (
        <div className="fixed inset-0 z-[60] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="w-full max-w-xs bg-gray-900 border border-white/10 rounded-[3rem] p-10 shadow-2xl scale-100">
            ...
            <div className="space-y-6 mt-10">
              ...
              <div className="flex flex-col gap-3">
                <button onClick={saveDeviceKey} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm tracking-widest uppercase shadow-xl hover:bg-indigo-700">Save</button>
                <button onClick={() => setShowSettings(false)} className="w-full text-gray-500 py-3 font-bold text-xs uppercase tracking-widest">Close</button>
              </div>
            </div>
          </div>
        </div>
