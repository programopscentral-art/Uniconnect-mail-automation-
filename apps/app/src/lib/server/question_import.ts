...
/**
 * Question-bank import for ONE subject.
 *
 * Lifted verbatim out of the questions/upload endpoint so the bulk
 * (university / global) uploader can reuse the exact same proven path instead of
 * forking it. Behaviour is unchanged: pass unitId 'GLOBAL' to let the sheet's own
 * Unit column route rows across a subject's units; units and topics are created
 * on demand.
 */
export interface QuestionImportOptions {
    fileName: string;
    arrayBuffer: ArrayBuffer;
    /** A unit id, or 'GLOBAL' to route rows by the sheet's Unit column. */
    unitId: string;
    subjectId: string;
    mode?: string;
    sheetName?: string;
...
export async function importQuestionBank(opts: QuestionImportOptions): Promise<{ count: number }> {
    const { unitId, subjectId } = opts;
    const mode = opts.mode || 'normal';
    const sheetName = opts.sheetName as string;
    ...
    if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
        ...
        for (const sName of sheetsToProcess) {
            ...
            for (const row of rows as any[]) {
                ...
                /*
                 * Work out the unit NUMBER first.
                 *
                 * A plain "Unit" column holding 1/2/3 used to fall through to the
                 * name matcher (the number-header list only had "Unit Number" /
                 * "Module Number"), match nothing, and land every question on
                 * Unit 1 — which quietly destroyed the spread across units. Accept
                 * a bare number, and "Unit 2" / "Module 3" too.
                 */
                let unitNum: number | null = null;
                if (rawModNum) {
                    const digitMatch = rawModNum.match(/\d+/);
                    const n = digitMatch ? parseInt(digitMatch[0]) : NaN;
                    if (!isNaN(n)) unitNum = n;
                }
                if (unitNum === null && rawModName) {
                    const m = rawModName.toString().trim().match(/^(?:unit|module)?\s*[-:.]?\s*(\d+)$/i);
                    if (m) unitNum = parseInt(m[1]);
                }

                if (unitNum !== null && !isNaN(unitNum)) {
                    unit = allUnits.find(u => Number(u.unit_number) === unitNum);
                    if (!unit) {
                        const res = await db.query(
                            'INSERT INTO assessment_units (subject_id, unit_number, name) VALUES ($1, $2, $3) RETURNING *',
                            [subjectId, unitNum, `Unit ${unitNum}`]
                        );
