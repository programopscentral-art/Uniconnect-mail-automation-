<script lang="ts">
  /**
   * SGU75SEMTemplate.svelte
   * SGU End Semester Examination — 75 Marks variant
   *
   * Layout-only specialization based on SGU50SEMTemplate.svelte.
   * All rendering/DnD/answer-sheet logic is reused verbatim.
   *
   * Paper structure (75M):
   *   Q1 : SINGLE slot, 5 sub-questions (a–e) × 3M = 15M — "Attempt the following Questions."
   *   Q2–Q5 : SINGLE slot, 4 sub-questions (a–d) × 5M each, attempt any 3 = 15M each
  ...
  // ── Reused verbatim from SGU50SEMTemplate ──────────────────────────────────
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-right">
              ...
              <AssessmentEditable
                value={paperMeta.duration_label || "03 hrs"}
          ...
          {#each paperStructure as section, sIdx}
            ...
            {#if sectionQuestions.length > 0}
                {#each sectionQuestions as slot, i (slot.id + activeSet)}
                  ...
                  {:else}
                    <!-- SINGLE slot — Q1 (5 sub-questions a–e) or Q2–Q5 (4 sub-questions a–d) -->
