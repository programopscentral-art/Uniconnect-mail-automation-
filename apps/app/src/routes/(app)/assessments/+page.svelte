<script lang="ts">
  ...
  async function loadBranches(batchId: string) {
    ...
    try {
      const res = await fetch(
        `/api/assessments/branches?universityId=${activeUniversity?.id}&batchId=${batchId}`,
      );
      if (res.ok) {
  ...
  async function loadSubjects(branchId: string) {
    ...
    try {
      const res = await fetch(
        `/api/assessments/subjects?branchId=${branchId}${selectedBatchId ? `&batchId=${selectedBatchId}` : ""}`,
      );
      if (res.ok) {
  ...
  async function addBatch() {
    if (!newBatchName || !activeUniversity?.id) {
      alert(
        "University context is missing. Please refresh or select a university.",
      );
      return;
    }
  ...
  async function deleteBatch(id: string) {
    ...
    const res = await fetch(`/api/assessments/batches?id=${id}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  async function deleteBranch(id: string) {
    ...
    const res = await fetch(`/api/assessments/branches?id=${id}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  async function deleteSubject(id: string) {
    ...
    const res = await fetch(`/api/assessments/subjects?id=${id}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  async function bulkDeleteSubjects() {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedSubjectIds.length} subjects?`,
      )
    )
      return;
...
<div class="space-y-8 animate-premium-fade">
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div class="animate-premium-slide">
      ...
      >
        Syllabus <span class="text-indigo-600 dark:text-indigo-400">&</span> Subjects
      </h1>
      <p class="text-sm font-medium text-gray-500 dark:text-slate-500 mt-2">
        Manage your batches, departments, subjects, and generate question
        papers.
      </p>
    </div>

    <div
      class="flex gap-4 animate-premium-slide"
  ...
  {#if data.universities.length > 0}
    <div
      class="glass p-6 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6 animate-premium-slide"
      style="animation-delay: 200ms;"
    >
      <div class="flex items-center gap-5">
        <div
          class="w-14 h-14 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-xl font-black border border-indigo-500 shadow-lg shadow-indigo-500/20 uppercase"
        >
          {activeUniversity?.name?.[0] || "U"}
        </div>
        <div>
          <h3
            class="text-xl font-black text-gray-900 dark:text-white leading-tight uppercase tracking-tighter"
          >
            {activeUniversity?.name || "Select Entity"}
          </h3>
          <p
            class="text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-[0.2em] mt-1"
          >
            Active University
          </p>
        </div>
      ...
      <div
        class="flex items-center gap-4 bg-white/50 dark:bg-slate-900/50 p-2 rounded-2xl border border-gray-100 dark:border-slate-800"
      >
        <span
          class="text-[9px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest ml-3"
          >Switch University</span
        >
        <select
        ...
        >
          {#each data.universities as univ}
            <option
              value={univ.id}
              selected={univ.id === data.selectedUniversityId}
              >{univ.name}</option
            >
          {/each}
        </select>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="space-y-6">
      <!-- Batches -->
      <div
        class="glass overflow-hidden rounded-[2.5rem] animate-premium-slide"
        style="animation-delay: 300ms;"
      >
        <div
          class="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50"
        >
          <div>
            <h3
            ...
            <p
              class="text-[9px] text-gray-400 dark:text-slate-600 font-bold mt-1 italic"
            >
              Class Batches
            </p>
          </div>
        ...
        <div class="p-4 space-y-3 max-h-[250px] overflow-y-auto">
          {#if showAddBatch}
            <div
              class="p-5 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800/50 space-y-4 shadow-inner"
              transition:slide
            >
              <input
                type="text"
          ...
          {#each data.batches as batch}
            <div
              ...
              class="w-full flex items-center justify-between p-4 rounded-2xl border transition-all text-left cursor-pointer group
                ...
                : 'bg-white/50 dark:bg-slate-800/30 border-gray-100 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500'}"
            >
              <span
                class="text-xs font-bold {selectedBatchId === batch.id
                  ? 'text-white'
                  : 'text-gray-900 dark:text-white'}">{batch.name}</span
              >
              <div
          ...
          {:else}
            <p
              class="text-[9px] text-gray-400 dark:text-slate-700 font-bold text-center py-10 uppercase tracking-widest italic"
            >
              No Batches
            </p>
          {/each}
      ...
      >
        ...
        <div class="p-4 space-y-3 max-h-[400px] overflow-y-auto">
          {#if !selectedBatchId}
            <div
              class="py-16 flex flex-col items-center justify-center text-center opacity-30"
            >
              <svg
              ...
              >
              <p
                class="text-[9px] font-black text-gray-900 dark:text-white uppercase tracking-[0.2em]"
              >
            ...
            {#each branches as branch}
              ...
              >
                <div class="flex flex-col">
                  <span
                    class="text-xs font-bold {selectedBranchId === branch.id
                      ? 'text-white'
                      : 'text-gray-900 dark:text-white'}">{branch.name}</span
                  >
                  <span
                    class="text-[9px] font-black {selectedBranchId === branch.id
                      ? 'text-indigo-100'
                      : 'text-indigo-400'} uppercase tracking-[0.1em] mt-1 italic"
                    >{branch.code}</span
                  >
                </div>
                <div
            ...
            {:else}
              {#if isLoadingBranches}
                <div class="flex items-center justify-center py-10">
                  <div
                    class="animate-spin rounded-full h-6 w-6 border-2 border-indigo-500 border-t-transparent shadow-sm"
                  ></div>
                </div>
              {:else}
                <p
    ...
    <div class="lg:col-span-2 space-y-6">
      <div
        class="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm min-h-[600px] flex flex-col"
      >
        <div
          class="p-8 border-b border-gray-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-gray-50/30 dark:bg-slate-800/20"
        >
          <div>
            <h2
              class="text-2xl font-black text-gray-900 dark:text-white leading-tight uppercase tracking-tighter"
            >
              Syllabus <span class="text-indigo-600 dark:text-indigo-400"
                >&</span
              > Subjects
            </h2>
            {#if selectedBatchId && selectedBranchId}
              <p
                class="text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-[0.2em] mt-2"
              >
                <span class="text-gray-400 dark:text-slate-400"
                  >{activeBatch?.name}</span
                >
                •
            ...
            {:else}
              <p
                class="text-[10px] font-black text-gray-400 dark:text-slate-700 uppercase tracking-[0.2em] mt-2 italic"
              >
                Awaiting structural context
              </p>
            {/if}
          </div>
        ...
        <div class="flex-1 p-8">
          {#if !selectedBatchId || !selectedBranchId}
            <div
              class="h-full flex flex-col items-center justify-center text-center py-24 animate-premium-fade"
            >
              <div
                class="w-24 h-24 glass rounded-[2.5rem] flex items-center justify-center text-indigo-500 mb-8 border border-white dark:border-slate-800 shadow-xl shadow-indigo-500/10"
              >
                <svg
                  class="w-12 h-12"
                  fill="none"
              ...
              <p
                class="text-sm text-gray-500 dark:text-gray-400 mt-4 max-w-sm font-medium leading-relaxed"
              >
                View subjects by selecting a <span
          ...
          {:else}
            ...
            {#if showAddSubject}
              <div
                class="mb-10 p-8 glass rounded-[2.5rem] border border-white dark:border-slate-800 shadow-xl shadow-indigo-500/5"
                transition:slide
              >
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                  ...
                  <div class="space-y-2">
                    ...
                    <select
                      id="subjectSemester"
                      bind:value={newSubjectSemester}
                      class="w-full bg-white dark:bg-slate-800 border-gray-100 dark:border-gray-700 rounded-2xl text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all px-4 py-3 text-gray-900 dark:text-white"
                    >
                      {#each [1, 2, 3, 4, 5, 6, 7, 8] as sem}
                        <option value={sem} class="dark:bg-slate-800"
                          >Semester {sem}</option
                        >
                      {/each}
                    </select>
                ...
                <div
                  class="flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-slate-800"
                >
                  <button
                    onclick={() => (showAddSubject = false)}
                    class="px-6 py-3 bg-white dark:bg-slate-900 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-xl border border-gray-200 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
                    >CANCEL</button
                  >
                  <button
                    onclick={addSubject}
                    class="px-8 py-3 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20"
                    >SAVE SUBJECT</button
                  >
                </div>
              </div>
            ...
            <div class="space-y-10">
              {#each [1, 2, 3, 4, 5, 6, 7, 8] as sem}
                {#if subjects.filter((s) => s.semester === sem).length > 0}
                  <div
                    class="space-y-6 animate-premium-slide"
                    style="animation-delay: {sem * 50}ms;"
                  >
                    <div class="flex items-center gap-5">
                      <div
                      ...
                      {#each subjects.filter((s) => s.semester === sem) as subject}
                        <div
                          class="group p-6 rounded-[2rem] border transition-all cursor-pointer relative overflow-hidden
                            ...
                            : 'bg-white dark:bg-slate-800/40 border-gray-100 dark:border-slate-800 shadow-sm hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-xl hover:shadow-indigo-500/5'}"
                        ...
                        >
                          ...
                          >
                            <div class="flex items-start gap-4">
                              ...
                              <div>
                                <h4
                                  class="text-base font-black leading-tight uppercase tracking-tight {selectedSubjectIds.includes(
                                    subject.id,
                                  )
                                    ? 'text-white'
                                ...
                                <p
                                  class="text-[10px] font-black uppercase tracking-widest mt-1 {selectedSubjectIds.includes(
                                    subject.id,
                                  )
                                    ? 'text-indigo-200'
                                    : 'text-indigo-500 dark:text-indigo-400'}"
                                >
                                  {subject.code || "UNMAPPED-ID"}
                                </p>
                              </div>
                            </div>
                            <div
                            ...
                            >
                              <button
                                ...
                                class="p-2 rounded-xl {selectedSubjectIds.includes(
                                  subject.id,
                                )
                                  ? 'text-white hover:bg-white/10'
                                  : 'text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20'} transition-all"
                              >
                                <svg
                                  class="w-4 h-4"
                                  fill="none"
                              ...
                              <button
                                ...
                                class="p-2 rounded-xl {selectedSubjectIds.includes(
                                  ...
                                  ? 'text-white hover:bg-white/10'
                                  : 'text-gray-300 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'} transition-all"
                              >
                                <svg
                                  class="w-4 h-4"
                                  fill="none"
                          ...
                          <div
                            class="mt-4 flex items-center justify-between relative z-10"
                          >
                            <div class="flex -space-x-2 overflow-hidden">
                              {#each [1, 2, 3, 4, 5] as unit}
                                <div
                                  class="inline-block h-7 w-7 rounded-xl ring-4 ring-white dark:ring-gray-800 bg-gray-50 dark:bg-slate-900 border border-gray-100 dark:border-gray-700 text-[10px] font-black flex items-center justify-center text-gray-400 dark:text-slate-400 shadow-sm"
                                >
                                  U{unit}
                                </div>
                              {/each}
                            ...
                            <a
                              ...
                              class="text-[11px] font-black uppercase tracking-widest {selectedSubjectIds.includes(
                                subject.id,
                              )
              ...
              {:else}
                <div
                  class="h-full flex flex-col items-center justify-center text-center py-24 opacity-30 animate-premium-fade"
                >
                  <div
                    class="w-20 h-20 bg-gray-50 dark:bg-slate-900 rounded-[2rem] flex items-center justify-center text-gray-300 dark:text-slate-700 mb-6 border border-gray-100 dark:border-slate-800 shadow-inner"
                  >
                    <svg
                      class="w-10 h-10"
                      fill="none"
                  ...
                  <h3
                    class="text-sm font-black text-gray-700 dark:text-slate-300 uppercase tracking-[0.2em]"
                  >
                    Resource Void
                  </h3>
                  <p
                    class="text-[10px] text-gray-400 dark:text-slate-600 font-bold mt-2 uppercase tracking-widest italic"
                  >
...
{#if showEditBatch && editingBatch}
  ...
  >
    <div
      class="glass rounded-[3rem] w-full max-w-md p-10 shadow-2xl border border-white dark:border-slate-800"
      ...
      <div class="space-y-6">
        <div class="space-y-2">
          ...
          <input
            ...
            class="w-full bg-white dark:bg-slate-800/50 border-gray-100 dark:border-gray-700 rounded-[1.5rem] text-xs font-bold focus:ring-8 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all px-5 py-4 text-gray-900 dark:text-white"
          />
        </div>
        <div class="flex gap-4 pt-4">
          <button
            onclick={() => (showEditBatch = false)}
            class="flex-1 py-4 bg-white dark:bg-slate-900 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-[1.5rem] border border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
            >CANCEL</button
          >
          <button
            onclick={updateBatch}
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          <input
            id="editBranchCode"
            type="text"
            bind:value={editingBranch.code}
            class="w-full bg-white dark:bg-slate-800/50 border-gray-100 dark:border-gray-700 rounded-[1.5rem] text-xs font-bold focus:ring-8 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all px-5 py-4 text-gray-900 dark:text-white"
          />
        </div>
        <div class="flex gap-4 pt-4">
          <button
            onclick={() => (showEditBranch = false)}
            class="flex-1 py-4 bg-white dark:bg-slate-900 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-[1.5rem] border border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
            >CANCEL</button
          >
          <button
...
{#if showEditSubject && editingSubject}
  ...
  >
    <div
      class="glass rounded-[3rem] w-full max-w-2xl p-10 shadow-2xl border border-white dark:border-slate-800"
      transition:fly={{ y: 30, duration: 500 }}
    >
      <h3
        class="text-2xl font-black text-gray-900 dark:text-white mb-8 uppercase tracking-tighter"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-2">
          ...
          <input
            id="editSubjectCode"
            type="text"
            bind:value={editingSubject.code}
            class="w-full bg-white dark:bg-slate-800/50 border-gray-100 dark:border-gray-700 rounded-[1.5rem] text-xs font-bold focus:ring-8 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all px-5 py-4 text-gray-900 dark:text-white"
          />
        </div>
        <div class="space-y-2">
          ...
          <select
            ...
            bind:value={editingSubject.semester}
            class="w-full bg-white dark:bg-slate-800/50 border-gray-100 dark:border-gray-700 rounded-[1.5rem] text-xs font-bold focus:ring-8 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all px-5 py-4 text-gray-900 dark:text-white"
          >
            {#each [1, 2, 3, 4, 5, 6, 7, 8] as sem}
              <option value={sem} class="dark:bg-slate-800"
      ...
      <div
        class="flex gap-4 pt-10 mt-6 border-t border-gray-100 dark:border-slate-800"
      >
        <button
          onclick={() => (showEditSubject = false)}
          class="flex-1 py-4 bg-white dark:bg-slate-900 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-[1.5rem] border border-gray-200 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
          >CANCEL</button
        >
        <button
...
{#if showSyncModal}
  ...
  >
    <div
      class="glass rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-2xl border border-white dark:border-slate-800"
      transition:fly={{ y: 30, duration: 500 }}
    >
      <div class="bg-indigo-600 p-10 text-white relative">
        <h3 class="text-3xl font-black uppercase tracking-tighter">
          Universal Sync Pipeline
        </h3>
        <p
          class="text-indigo-100 text-xs font-bold uppercase tracking-[0.2em] mt-2 opacity-80"
        >
          Autonomous syllabus replication across institution nodes
        </p>
        <button
          onclick={() => (showSyncModal = false)}
          class="absolute top-10 right-10 text-white/50 hover:text-white transition-all transform hover:rotate-90"
      ...
      <div
        class="p-10 bg-gray-50/50 dark:bg-slate-900/30 border-t border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6"
      >
        <div
          class="flex items-center gap-3 bg-white dark:bg-slate-800 px-4 py-2 rounded-xl border border-gray-100 dark:border-gray-700"
        >
          <div class="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></div>
          <p
            class="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest"
          >
            Selected: <span class="text-indigo-600 dark:text-indigo-400"
              >{syncTargetUniIds.length} Nodes</span
            >
          </p>
        </div>
        <div class="flex gap-4 w-full sm:w-auto">
          <button
            onclick={() => (showSyncModal = false)}
            class="flex-1 sm:flex-none px-8 py-4 bg-white dark:bg-slate-900 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-2xl border border-gray-200 dark:border-slate-800 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all"
            >ABORT</button
          >
          <button
