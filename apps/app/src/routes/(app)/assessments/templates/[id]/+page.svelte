...
{:else}
  <div
    ...
    class="h-screen flex flex-col bg-white dark:bg-slate-950 overflow-hidden"
  >
    <!-- Header -->
    <header
    ...
    >
      ...
      <div class="flex items-center gap-3">
        <button
          ...
          class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-2 {studioMode
            ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-600/20'
            : 'text-indigo-600 border-indigo-100 hover:bg-indigo-50 dark:border-indigo-900/30'}"
        >
          ...
          {:else if activeTab === "components"}
            <div in:fade={{ duration: 200 }} class="space-y-8">
              ...
              <div class="grid grid-cols-1 gap-4">
                ...
                <button
                  onclick={() => {
                    const part = String.fromCharCode(
                      65 + template.config.length,
                    );
                    template.config = [
                      ...template.config,
                      {
