<script lang="ts">
  ...
  // Friendly date range label, e.g. "May 20 → May 26, 2026"
  let prettyRange = $derived.by(() => {
  ...
  <div class="mx-auto max-w-3xl px-4 py-6">
    ...
    <div class="mb-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
      <div class="flex items-start justify-between gap-3">
        <div>
          <div class="text-[10px] uppercase tracking-[0.18em] text-zinc-500">Weekly Summary · NIAT</div>
          <div class="mt-1 text-lg font-semibold">
            {data.campuses.find(c => c.campus_id === data.activeCampusId)?.display_name ?? '—'}
          </div>
          <div class="mt-0.5 text-sm text-zinc-400">{prettyRange}</div>
          <div class="mt-0.5 text-xs text-zinc-500 tabular-nums">{data.period_start} → {data.period_end} (Mon → Sun IST)</div>
        </div>
      </div>
    ...
    {#if data.campuses.length === 0}
      <div class="rounded-lg border border-amber-800 bg-amber-950/30 p-4 text-sm text-amber-200">
        No campuses available. Contact your PM/admin.
      </div>
    {:else if !data.rollup}
      <div class="rounded-lg border border-zinc-800 bg-zinc-900 p-4 text-sm text-zinc-400">
        Pick a campus to load the weekly summary.
      </div>
    {:else}
      {@const r = data.rollup}
      ...
      <div class="mt-4 rounded-md border border-zinc-800 bg-zinc-900/50 px-4 py-2 text-xs text-zinc-500">
        All numbers are computed from this campus's daily reports for the period above.
        Nothing to fill, nothing to submit — change the campus or week to load a different rollup.
      </div>
    {/if}
  </div>
</div>
