<script lang="ts">
  ...
  let form = $state({
    university_id: data.proposal.university_id,
    title: data.proposal.title,
    event_type: data.proposal.event_type,
    proposed_date: data.proposal.proposed_date ? new Date(data.proposal.proposed_date).toISOString().slice(0, 16) : "",
    venue: data.proposal.venue || "",
    expected_attendance: data.proposal.expected_attendance || 0,
    description: data.proposal.description || "",
    priority: data.proposal.priority || "MED",
    items: data.proposal.items.length > 0 ? JSON.parse(JSON.stringify(data.proposal.items)) : [
  ...
  <div class="flex items-center justify-between mb-12">
    <div>
      ...
      <p class="text-sm font-bold text-amber-600 uppercase tracking-widest mt-1">
        Current Status: {proposal.status.replace('_', ' ')}
      </p>
    </div>
      ...
      <div in:fade={{ duration: 200 }} class="space-y-8">
        <div class="border-b border-gray-100 dark:border-gray-700 pb-6">
          ...
          <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">
            Update the core parameters of your event.
          </p>
        </div>
      ...
      <div in:fade={{ duration: 200 }} class="space-y-8">
        ...
        >
          <div>
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">
              Revise itemized expenses as needed.
            </p>
          </div>
    ...
    {:else if currentStep === 3}
      <div in:fade={{ duration: 200 }} class="space-y-8">
        <div class="border-b border-gray-100 dark:border-gray-700 pb-6">
          ...
          <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">
            Explain your changes or provide additional context.
          </p>
        </div>
    ...
    >
      ...
      >
        {#if isSubmitting}
          ...
          Saving...
        {:else}
          {currentStep === 3 ? "Save Changes" : "Continue"}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-5 h-5"
            fill="none"
