<script lang="ts">
    ...
    async function loadSheets() {
        ...
        try {
            ...
            sheets = (json.sheets || []).filter((s: any) => !s.spreadsheet_id.startsWith('pending_'));
        } catch (e) {
    ...
    function getEmbedUrl(sheetUrl: string) {
        // Convert edit URL to embed URL
        if (sheetUrl.includes('/edit')) {
            return sheetUrl.replace('/edit', '/htmlembed');
        }
        return sheetUrl + '/htmlembed';
    }
    ...
    // Simple markdown-like rendering for AI analysis
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Connect Google Sheets, sync data, get AI insights</p>
            </div>
                ...
                <button onclick={connectGoogle} disabled={connecting}
                    ...
                    {:else}
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 0C5.372 0 0 5.372 0 12s5.372 12 12 12 12-5.372 12-12S18.628 0 12 0zm6.36 18.36c-1.68 1.68-3.96 2.64-6.36 2.64s-4.68-.96-6.36-2.64C3.96 16.68 3 14.4 3 12s.96-4.68 2.64-6.36C7.32 3.96 9.6 3 12 3s4.68.96 6.36 2.64C20.04 7.32 21 9.6 21 12s-.96 4.68-2.64 6.36z"/>
                        </svg>
                    {/if}
        ...
        {:else if sheets.length === 0}
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-12 text-center">
                ...
                <h2 class="text-xl font-bold text-slate-900 dark:text-white mb-2">Connect Your First Google Sheet</h2>
                <p class="text-slate-500 dark:text-slate-400 mb-6 max-w-md mx-auto">
                    Link Google Sheets to UniConnect. View them inline, sync data automatically, and get AI-powered insights and suggestions.
                </p>
                <div class="flex flex-col sm:flex-row gap-3 justify-center">
                    <button onclick={connectGoogle} disabled={connecting}
                        class="px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex items-center gap-2 justify-center disabled:opacity-50">
                        {#if connecting}
                            <div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                            Connecting...
                        {:else}
                ...
                <div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <p class="text-xs font-semibold text-slate-700 dark:text-slate-300">View Inline</p>
                        <p class="text-[10px] text-slate-500">Edit sheets without leaving UniConnect</p>
                    </div>
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <p class="text-xs font-semibold text-slate-700 dark:text-slate-300">Sync Data</p>
                        <p class="text-[10px] text-slate-500">Pull sheet data into UniConnect backend</p>
                    </div>
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
        ...
        {:else}
            ...
            <div class="flex gap-4" style="height: calc(100vh - 160px);">
                <!-- Left: Sheet List -->
                <div class="w-64 shrink-0 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
                    <div class="p-3 border-b border-slate-100 dark:border-slate-800">
                        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Connected Sheets</p>
                    </div>
                            ...
                            <div role="button" tabindex="0" onclick={() => selectSheet(sheet)} onkeydown={(e) => { if (e.key === 'Enter') selectSheet(sheet); }}
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-[10px] text-slate-400 mt-0.5">{sheet.google_email}</p>
                ...
                <div class="flex-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
                    {#if selectedSheet}
                        ...
                        <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-400">{selectedSheet.google_email}</p>
                                </div>
                            </div>
                        ...
                        <!-- Sync Info Bar -->
                        {#if syncData}
                            <div class="px-4 py-2 bg-emerald-50 dark:bg-emerald-900/10 border-b border-emerald-100 dark:border-emerald-900/20 flex items-center justify-between">
                                <div class="flex items-center gap-4 text-xs text-emerald-700 dark:text-emerald-400">
                                    <span class="font-semibold">{syncData.rowCount} rows synced</span>
                                    <span>{syncData.headers?.length || 0} columns</span>
                                    {#if selectedSheet.last_synced_at}
                                        <span>Last sync: {formatDate(selectedSheet.last_synced_at)}</span>
                                    {/if}
                                </div>
                            </div>
                        {/if}
                        ...
                        {#if selectedSheet.sheet_url}
                            <div class="flex-1">
                                <iframe
                                    src={getEmbedUrl(selectedSheet.sheet_url)}
                    ...
                    {:else}
                        ...
                        <div class="flex-1 flex items-center justify-center">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-slate-400">Select a sheet to view</p>
                            </div>
                        </div>
                    {/if}
                </div>
    ...
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md w-full space-y-4 border border-slate-200 dark:border-slate-700 shadow-xl">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Link Google Sheet</h3>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Sheet URL *</label>
                <input type="url" bind:value={linkUrl} placeholder="https://docs.google.com/spreadsheets/d/..."
                    class="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"/>
            </div>
            ...
            <p class="text-[10px] text-slate-400 text-center">
                Make sure the sheet is shared with the Google account you connected.
            </p>
        </div>
    </div>
{/if}
