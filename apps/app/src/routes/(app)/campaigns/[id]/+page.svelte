<script lang="ts">
  ...
  async function loadRecipients(force = false, filterToSet?: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log(
          `[UI] Received ${delta.length} recipients (isDelta: ${force && !!lastRecipientUpdate})`,
        );
      ...
      alert(
        "Failed to load recipient list. Please check your connection or redeploy.",
      );
    } finally {
  ...
  async function retryFailed() {
    ...
    try {
      const res = await fetch(
        `/api/campaigns/${data.campaign.id}/retry-failed`,
        { method: "POST" },
      );
      if (res.ok) {
        alert("Retry started!");
  ...
  function openEdit(r: any) {
    if (!r.metadata || Object.keys(r.metadata).length === 0) {
      console.warn(
        `[UI] Recipient ${r.id} has no metadata in current view record.`,
      );
    }
    editingRecipient = {
      ...r,
      metadata_str: JSON.stringify(r.metadata || {}, null, 2),
    };
    showEditModal = true;
  }
  ...
  async function saveStudentEdit() {
    if (
      !editingRecipient.to_email ||
      !editingRecipient.to_email.includes("@")
    ) {
    ...
    try {
      ...
      if (
        ...
        Object.keys(editingRecipient.metadata || {}).length > 0
      ) {
        if (
          !confirm(
            "⚠️ WARNING: You are saving EMPTY metadata. This will delete all fee information and other custom fields for this student. Continue?",
          )
        ) {
          isSavingEdit = false;
          return;
      ...
      const res = await fetch(
        `/api/campaigns/${data.campaign.id}/recipients/${editingRecipient.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function scheduleCampaign() {
    ...
    if (!isLater) {
      if (
        !confirm(
          "🚀 CONFIRM: Are you sure you want to start sending emails to all recipients now? This action cannot be undone.",
        )
      )
        return;
    } else {
  ...
  async function stopCampaign() {
    if (
      !confirm(
        "🛑 Are you sure you want to STOP this campaign? Emails already in the queue will be cancelled.",
      )
    )
      return;
    try {
      const res = await fetch(`/api/campaigns/${data.campaign.id}/stop`, {
        method: "POST",
      });
      if (res.ok) {
  ...
  async function resumeCampaign() {
    if (
      !confirm(
        "▶️ Are you sure you want to RESUME this campaign? Emails will start sending to remaining recipients.",
      )
    )
      return;
    try {
      const res = await fetch(`/api/campaigns/${data.campaign.id}/resume`, {
        method: "POST",
      });
      if (res.ok) {
  ...
  const progress = $derived.by(() => {
    ...
    if (total === 0) return 0;
    const processed =
      (data.campaign.sent_count || 0) + (data.campaign.failed_count || 0);
...
<div class="space-y-6">
  <div
    class="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-slate-800"
  >
    <div>
      ...
      {#if data.campaign.status === "STOPPED" || data.campaign.status === "COMPLETED"}
        ...
        >
          <svg
            ...
            ><path
              fill-rule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM2 10a8 8 0 018-8v8h8a8 8 0 11-16 0z"
              clip-rule="evenodd"
      ...
      {#if data.campaign.status === "DRAFT"}
        ...
        {:else}
          <div class="flex items-center space-x-2">
            type="datetime-local" bind:value={scheduledAt}
            class="block w-48 pl-3 pr-10 py-2 text-sm border-gray-300 focus:outline-none
            focus:ring-blue-500 focus:border-blue-500 rounded-md text-gray-900 bg-white"
            />
            <button
              onclick={scheduleCampaign}
            ...
            <button
              onclick={() => (showScheduleInput = false)}
              class="text-sm text-gray-500 font-bold">Cancel</button
            >
          </div>
        {/if}
      {/if}
  ...
  {#if data.campaign.status === "DRAFT"}
    <div
      class="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 shadow-sm"
    >
      <h2 class="text-xl font-bold mb-2" style="color: #312e81 !important">
        Production Readiness Check 🧪
      </h2>
      <p class="text-sm mb-5 font-bold" style="color: #312e81 !important">
        Verify your layout, NIAT branding, and dynamic placeholders using real
        student data before the official launch.
      </p>
    ...
    >
      <div class="flex justify-between items-end">
        <div>
          ...
          >
            {progress}%
            <span class="text-sm font-bold" style="color: #9ca3af !important"
              >({(data.campaign.sent_count || 0) +
                (data.campaign.failed_count || 0)} / {data.campaign
                .total_recipients})</span
            >
          </div>
        </div>
      ...
      <div
        class="w-full bg-gray-100 h-5 rounded-full overflow-hidden border border-gray-50 shadow-inner relative"
      >
        <div
  ...
  <!-- Available Students (shown when DRAFT) -->
  {#if data.campaign.status === 'DRAFT' && data.availableStudents !== undefined}
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          <p class="text-sm font-bold text-gray-500 uppercase">Students Loaded for this University</p>
          <p class="text-2xl font-black" style="color: #111827 !important">{data.availableStudents.toLocaleString()}</p>
        </div>
      </div>
      {#if data.availableStudents === 0}
        <span class="px-4 py-2 bg-red-50 text-red-600 text-xs font-black rounded-xl border border-red-100">No students uploaded — campaign cannot start</span>
      {:else}
        <span class="px-4 py-2 bg-green-50 text-green-600 text-xs font-black rounded-xl border border-green-100">Ready to send</span>
      {/if}
    </div>
  {/if}
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
    <button
      ...
      class="bg-white p-6 rounded-2xl shadow-sm border {statusFilter === 'ALL'
        ...
        : 'border-gray-100'} hover:border-indigo-200 transition-all text-left group"
    >
      <h3 class="text-sm font-bold uppercase" style="color: #6b7280 !important">
        Total Recipients
      </h3>
      <p class="mt-2 text-3xl font-black" style="color: #111827 !important">
        {data.campaign.status === 'DRAFT' ? (data.availableStudents ?? 0) : data.campaign.total_recipients}
      </p>
    </button>
    <button
      onclick={() => loadRecipients(false, "SENT")}
      class="bg-white p-6 rounded-2xl shadow-sm border {statusFilter === 'SENT'
        ? 'border-blue-500 ring-4 ring-blue-50'
        : 'border-gray-100'} hover:border-blue-200 transition-all text-left group"
    >
      <h3 class="text-sm font-bold uppercase" style="color: #6b7280 !important">
        Sent
      </h3>
    ...
    <button
      ...
      'ACKNOWLEDGED'
        ...
        : 'border-gray-100'} hover:border-green-200 transition-all text-left group"
    >
      <h3 class="text-sm font-bold uppercase" style="color: #6b7280 !important">
        Acknowledged
      </h3>
      <p class="mt-2 text-3xl font-black text-green-600">
        {data.campaign.ack_count || 0}
      </p>
    </button>
    <button
      ...
      'FAILED'
        ...
        : 'border-gray-100'} hover:border-red-200 transition-all text-left relative group"
    >
      <h3 class="text-sm font-bold uppercase" style="color: #6b7280 !important">
        Failed
      </h3>
      <div class="flex items-end justify-between">
        <p class="mt-2 text-3xl font-black text-red-600">
          {data.campaign.failed_count || 0}
        </p>
        {#if data.campaign.failed_count > 0}
          <div
            class="text-[10px] font-black text-red-500 uppercase tracking-widest animate-bounce"
          >
            Click to view ↴
          </div>
        {/if}
  ...
  <!-- Recipients -->
  <div
    class="bg-white shadow-2xl rounded-[2.5rem] overflow-hidden border border-gray-100"
  >
    <div class="px-8 py-6 flex justify-between items-center bg-gray-50/50">
      <div class="flex-1">
        <div class="flex items-center gap-3">
          ...
          <span
            class="px-2 py-0.5 rounded-full bg-red-100 text-red-700 text-[10px] font-black border border-red-200 animate-pulse"
            >BUILD: V6.0-DEBUG-ENABLED</span
          >
        </div>
            ...
            {#each ["ALL", "SENT", "ACKNOWLEDGED", "FAILED", "SKIPPED"] as filter}
              <button
                ...
                class="px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all
                  ...
                  : 'bg-white text-gray-700 border border-gray-100 hover:border-indigo-200 font-bold'}"
              >
                {filter}
              </button>
            {/each}
        ...
        <div class="mt-4">
          <div class="relative max-w-md">
            ...
            />
            <svg
              class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-500"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              ><path
                ...
                {#each filteredRecipients as r}
                  <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      ...
                      {#if r.error_message}
                        <p
                          class="text-xs text-red-500 mt-1 max-w-xs overflow-hidden text-ellipsis"
                        >
                          {r.error_message}
                        </p>
                      {/if}
    ...
    >
      ...
      >
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <h3
            class="text-lg leading-6 font-black mb-4"
            style="color: #111827 !important"
          >
            Edit Student Data
          </h3>
          ...
          <div class="space-y-4">
            <div>
              ...
              <input
                id="edit-name"
                type="text"
                bind:value={editingRecipient.student_name}
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm text-gray-900 font-medium px-3 py-2"
              />
            </div>
            <div>
              <label
              ...
              <input
                id="edit-email"
                type="email"
                bind:value={editingRecipient.to_email}
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm text-gray-900 font-medium px-3 py-2"
              />
            </div>
            <div>
              ...
              <textarea
                id="edit-meta"
                bind:value={editingRecipient.metadata_str}
                rows="10"
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm font-mono text-xs text-gray-900 px-3 py-2"
              ></textarea>
              <p class="mt-1 text-xs dark:text-slate-400 italic">
                This will update the student's record and all future mails sent
                to them.
              </p>
            </div>
          </div>
