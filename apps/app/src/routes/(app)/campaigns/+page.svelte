...
<div class="space-y-8 animate-premium-fade">
  <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
    <div class="animate-premium-slide">
      ...
      <p class="mt-2 text-sm font-medium text-gray-500 dark:text-gray-400">Monitor and manage email campaigns.</p>
    </div>
    <a 
  ...
  {#if (data.user?.permissions || []).includes('universities') || data.universities.length > 1}
    <div class="glass p-6 rounded-[2.5rem] flex items-center gap-6 animate-premium-slide" style="animation-delay: 200ms;">
        <div class="hidden sm:flex items-center space-x-3">
            ...
            <label for="univ-select" class="text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest">University</label>
        </div>
        <select 
            id="univ-select" 
    ...
    {#each groupedCampaigns as [date, items], i}
      <div class="space-y-6 animate-premium-slide" style="animation-delay: {300 + (i * 100)}ms;">
        <div class="flex items-center gap-6">
          ...
          <span class="text-[10px] font-black text-gray-400 dark:text-gray-600 uppercase tracking-widest italic">{items.length} Campaigns</span>
        </div>
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
              <thead class="bg-gray-50/50 dark:bg-slate-800/50">
                <tr>
                  <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest">Campaign Name</th>
                  <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest">Status</th>
                  <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest">Template Used</th>
                  <th class="px-8 py-5 text-left text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest">Progress</th>
                ...
                {#each items as cam}
                  <tr class="group hover:bg-slate-950/50 dark:hover:bg-gray-800/30 transition-colors">
                    <td class="px-8 py-6 whitespace-normal max-w-xs">
                      ...
                      <div class="text-[9px] font-black text-gray-400 dark:text-slate-400 mt-1 uppercase tracking-tighter italic">ID: {cam.id.slice(0,8)}</div>
                    </td>
                    <td class="px-8 py-6 whitespace-nowrap">
                      <span class="px-4 py-1.5 text-[9px] font-black rounded-lg uppercase tracking-widest shadow-sm border
                        ...
                        {cam.status === 'IN_PROGRESS' ? 'PROCESSING' : cam.status === 'COMPLETED' ? 'COMPLETED' : cam.status}
                      </span>
                    </td>
                    <td class="px-8 py-6 whitespace-nowrap">
    ...
    {:else}
      <div class="px-8 py-32 text-center glass rounded-[2.5rem] animate-premium-slide" style="animation-delay: 300ms;">
          <div class="flex flex-col items-center space-y-6">
              ...
              <p class="text-[10px] font-black text-gray-400 dark:text-gray-600 uppercase tracking-[0.3em]">No campaigns found.</p>
          </div>
      </div>
    {/each}
  </div>
