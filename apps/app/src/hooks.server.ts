...
export const handle: Handle = async ({ event, resolve }) => {
    ...
    const isPublic = path.startsWith('/api/auth') || path === '/login' || path === '/accept-invite' || path.startsWith('/track') || path.startsWith('/ack') || path.startsWith('/api/documents/serve') || path === '/api/health' || path.startsWith('/api/ops/view-report') || path.startsWith('/api/ops/full-report');
